...
이 문서는 현재 `main` 작업 트리를 기준으로 한다. 특정 과거 커밋이나 배포 상태가 아니라 저장소의 최신 코드와 함께 갱신한다.
...
1. 학생이 실제 RAG 카탈로그에서 학교급·학년·단원·세부 성취기준을 선택한다.
2. 선택 과목과 성취기준 코드가 일치하는 근거 질문을 제시한다.
...
1. 사용자가 퀴즈 과목과 학교급·학년·단원·세부 성취기준을 선택한다.
2. 선택한 과목과 성취기준 코드가 모두 일치하는 RAG 질문·정답을 찾는다.
...
`scripts/build_curriculum_catalog.py`는 Chroma 본문의 `성취기준2022`를 전수 집계해 `app/resources/curriculum_catalog.json`을 만든다. 프런트는 이 파일을 제공하는 `/rag/catalog` API로 학교급·학년·단원·세부단원 선택지를 구성한다. 선택한 주제에는 `[성취기준 코드]`가 포함되며 Chroma의 `where_document`와 관계형 DB 폴백 검색 모두 같은 코드를 필수 조건으로 적용한다.
...
| GET | `/rag/catalog?subject=수학` | 실제 RAG 기반 단원·성취기준 카탈로그 |
