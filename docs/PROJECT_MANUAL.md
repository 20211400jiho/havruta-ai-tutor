...
1. 학생이 학습 주제를 선택한다.
2. 등록된 자료가 있으면 근거 질문을, 없으면 일반 하브루타 질문을 제시한다.
...
1. 사용자가 퀴즈 과목과 주제를 입력한다.
2. 선택한 과목의 RAG에서 관련 질문·정답을 찾는다.
...
Git에 포함된 원본 자료는 고1 수학 JSON 10건이다. 개발자 PC의 `chroma_db/`에는 9개 과목의 별도 인덱스가 있고 모두 2022 성취기준 매핑 검색을 통과했지만, Git 또는 Railway Docker 이미지로 자동 배포되지 않는다. 다른 환경에서 전 과목을 운영하려면 해당 인덱스를 영속 볼륨이나 외부 벡터 DB로 배포해야 한다.

### 17.3 벡터 검색

API에는 Chroma 우선 검색과 관계형 DB 어휘 검색 폴백이 통합돼 있다. 다만 Railway 이미지에는 로컬 `chroma_db/`가 포함되지 않으므로 운영 의미 검색을 사용하려면 영속 볼륨이나 외부 벡터 DB 구성이 필요하다.
