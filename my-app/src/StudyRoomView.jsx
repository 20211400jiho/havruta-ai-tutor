  ...
  const createRoom = async (event) => {
    ...
    try {
      const result = await api("/rooms", {
        method: "POST",
        body: JSON.stringify({ title, subject: "수학", grade: "고등학교 1학년", max_members: 4 }),
      });
    ...
    <main className="study-room-page">
      <div className="room-panel">
        ...
        <div className="room-actions">
          <form onSubmit={createRoom}><h3>새 학습방</h3><input value={title} onChange={(event) => setTitle(event.target.value)} required /><button className="btn-primary">생성하기</button></form>
